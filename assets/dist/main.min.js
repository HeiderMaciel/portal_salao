...
p=g("accent-height,accumulate,additive,alphabetic,arabic-form,ascent,attributeName,attributeType,baseProfile,bbox,begin,by,calcMode,cap-height,class,color,color-rendering,content,cx,cy,d,dx,dy,descent,display,dur,end,fill,fill-rule,font-family,font-size,font-stretch,font-style,font-variant,font-weight,from,fx,fy,g1,g2,glyph-name,gradientUnits,hanging,height,horiz-adv-x,horiz-origin-x,ideographic,k,keyPoints,keySplines,keyTimes,lang,marker-end,marker-mid,marker-start,markerHeight,markerUnits,markerWidth,mathematical,max,min,offset,opacity,orient,origin,overline-position,overline-thickness,panose-1,path,pathLength,points,preserveAspectRatio,r,refX,refY,repeatCount,repeatDur,requiredExtensions,requiredFeatures,restart,rotate,rx,ry,slope,stemh,stemv,stop-color,stop-opacity,strikethrough-position,strikethrough-thickness,stroke,stroke-dasharray,stroke-dashoffset,stroke-linecap,stroke-linejoin,stroke-miterlimit,stroke-opacity,stroke-width,systemLanguage,target,text-anchor,to,transform,type,u1,u2,underline-position,underline-thickness,unicode,unicode-range,units-per-em,values,version,viewBox,visibility,width,widths,x,x-height,x1,x2,xlink:actuate,xlink:arcrole,xlink:role,xlink:show,xlink:title,xlink:type,xml:base,xml:lang,xml:space,xmlns,xmlns:xlink,y,y1,y2,zoomAndPan");var P=h.extend({},D,p,n),q=document.createElement("pre"),M=/^(\s*)([\s\S]*?)(\s*)$/;h.module("ngSanitize",[]).provider("$sanitize",function(){this.$get=["$$sanitizeUri",function(a){return function(d){var c=[];F(d,s(c,function(c,b){return!/^unsafe/.test(a(c,b))}));return c.join("")}}]});h.module("ngSanitize").filter("linky",["$sanitize",function(a){var d=/((ftp|https?):\/\/|(mailto:)?[A-Za-z0-9._%+-]+@)\S*[^\s.;,(){}<>"]/,c=/^mailto:/;return function(e,b){function k(a){a&&g.push(E(a))}function f(a,c){g.push("<a ");h.isDefined(b)&&g.push('target="',b,'" ');g.push('href="',a.replace(/"/g,"&quot;"),'">');k(c);g.push("</a>")}if(!e)return e;for(var m,l=e,g=[],n,p;m=l.match(d);)n=m[0],m[2]==m[3]&&(n="mailto:"+n),p=m.index,k(l.substr(0,p)),f(n,m[0].replace(c,"")),l=l.substring(p+m[0].length);k(l);return a(g.join(""))}}])})(window,window.angular);var PortalApp=angular.module("PortalApp",["ngSanitize"]);PortalApp.parseRequest=function(data){data=data.substring(0,data.length-1);return JSON.parse(data)};PortalApp.serviceUrl="http://ebelle.vilarika.com.br/site";PortalApp.controller("PageUnitController",["$scope","$http","$location",function($scope,$http,$location){$http.get(PortalApp.serviceUrl+"/unitByName?name="+$location.$$path.replace("/","")).then(function(repose){$scope.unit=PortalApp.parseRequest(repose.data);setTimeout(function(){initializeMap()},500)})}]);PortalApp.controller("UnitController",["$scope","$http",function($scope,$http){var buildRows=function(data,nIntens,nRowToPage){var pages=[];var rows=[];var row=[];data.forEach(function(item,i){if(i%nIntens===0){if(row.length>0){rows.push(row);row=[]}if(rows.length===nRowToPage){pages.push({rows:rows});rows=[]}}row.push(item)});if(row.length){rows.push(row)}if(rows.length){pages.push({rows:rows})}return pages};$http.get(PortalApp.serviceUrl+"/unities").then(function(repose){$scope.pages=buildRows(PortalApp.parseRequest(repose.data),4,3);$scope.page=$scope.pages[0]});$scope.chagePage=function(page){$scope.page=page};$scope.nextPage=function(){var currentIndex=$scope.pages.indexOf($scope.page);currentIndex++;if(currentIndex===$scope.pages.length){currentIndex=0}$scope.page=$scope.pages[currentIndex]};$scope.prevPage=function(){var currentIndex=$scope.pages.indexOf($scope.page);currentIndex--;if(currentIndex===-1){currentIndex=$scope.pages.length}$scope.page=$scope.pages[currentIndex]}}]);